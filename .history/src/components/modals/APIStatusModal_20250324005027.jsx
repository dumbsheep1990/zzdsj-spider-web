...
function APIStatusModal({ visible, onCancel }) {
    ...
    const cloudApiItems = [
        {
            title: 'OpenAI API',
            status: settings.apiStatus?.openai || 'unknown',
            description: 'LLM内容提取和智能清洗服务 (GPT-3.5/GPT-4)',
            key: 'openai'
        },
        {
            title: 'Anthropic API',
            status: settings.apiStatus?.anthropic || 'unknown',
            description: 'LLM内容提取和智能清洗服务 (Claude)',
            key: 'anthropic'
        }
    ];
    ...
    const systemApiItems = [
        ...
        {
            title: 'Crawl4AI API',
            status: settings.apiStatus?.crawl4ai || 'unknown',
            description: '智能爬虫服务',
            key: 'crawl4ai'
        }
    ];
...
响应时间: OpenAI(${diagnosticData.responseTimes.openai}ms), Ollama(${diagnosticData.responseTimes.ollama}ms), Backend(${diagnosticData.responseTimes.backend}ms)
